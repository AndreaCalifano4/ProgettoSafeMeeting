...
/**
 * Servlet implementation class ServletAccount Questa servlet serve per gestire
 * l'intero reparto Account, ovvero modifica account sia docente che studente e
 * elimina account sia docente che studente.
 */
...
public class ServletAccount extends HttpServlet {
  ...
  private static final String SAVE_DIR = "C:\\Users\\emili\\git\\ProgettoSafeMeeting"
      + "\\ProgettoSafeMeeting\\WebContent\\bootstrap\\images";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response) 
    ...
    if (sb != null) {
      ...
      if (flag.equals("visualizza")) { // DA HOME STUDENTE AD ACCOUNT
        ...
        try {
          RequestDispatcher requestDispatcher = 
              request.getRequestDispatcher("VisualizzaDatiStudente.jsp");
      ...
      if (flag.equals("salvaModifica")) { // SALVA MODIFICA DEI DATI
        ...
        try {
          ...
          StudenteBean stud = studDao.modificaAccount(
              nome, cognome, matricola, oldPasswordEn, newPasswordEn, email);
          ...
          if (stud != null) {
            ...
            RequestDispatcher requestDispatcher = 
                request.getRequestDispatcher("AccountStudente.jsp");
            requestDispatcher.forward(request, response);
          }
    ...
    if (db != null) {
      ...
      if (flag1.equals("visualizzaDoc")) { // DA HOME DOCENTE QUANDO CLICCA SU ACCOUNT
        ...
        try {
          RequestDispatcher requestDispatcher = 
              request.getRequestDispatcher("VisualizzaDatiDocente.jsp");
          requestDispatcher.forward(request, response);
        } catch (Exception e) {
          e.printStackTrace();
        }
      ...
      if (flag1.equals("salvaModificaDoc")) { // QUANDO SI SALVANO I DATI MODIFICATI
        ...
        try {
          ...
          List<Part> fileParts = request.getParts()
              .stream().filter(part -> "file".equals(part.getName()))
              .collect(Collectors.toList());

          if (!fileParts.get(0)
              .getSubmittedFileName().isEmpty()) { /* CONTROLLA SE E' STATO EFFETTUATO UN UPLOAD */
            ...
            db.setImmagine(db.getImmagine()); /* SETTA IMMAGINE CHE E' GIA' PRESENTE */

            String immagine = db.getImmagine();

            DocenteBean doc = docDao.modificaAccount(
                nome, cognome, matricola, studio, oldPasswordEn, newPasswordEn,
                immagine, email);
            ...
            if (doc != null) {
              ...
              request.setAttribute("error", " ");
              RequestDispatcher requestDispatcher =
                  request.getRequestDispatcher("AccountDocente.jsp");
              requestDispatcher.forward(request, response);
            }
          }
        } catch (Exception e) {
