...
public class DocenteDao {
  ...
  /**
   * Questo metodo mi permettere di inserire un nuovo docente all'interno del db
   * relazionale.
   * 
   * @param db (DocenteBean)
   * @return true (nel try, nel caso in cui l'ha inserito con successo) oppure
   *         false(nel catch, nel caso in cui l'inserimento non è avvenuto con
   *         successo).
   */
   ...
   * @return db(DocenteBean, nel try, nel caso in cui viene effettuato l'accesso)
   *         oppure null (altrimenti), SQLException (nel catch, altrimenti)
   */
   ...
   * @param d            (data)
   * @return true (nel try, in caso sia assente) oppure false (nel catch,
   *         altrimenti)
   */
  
  public synchronized boolean assenzaDocente(
      String matricolaDoc, Time tempo1, Time tempo2, Date d) {
    ...
    try {
      ...
      ps = conn.prepareStatement(
          "DELETE FROM prenota "
          + "WHERE prenotaMatricolaDoc = ? AND giorno = ? AND orario between ? AND ? ;");
  ...
  /**
   * Questo metodo serve per ricavarci tutti gli studenti a cui verrà eliminata la
   * prenotazione.
   * 
   * @param db (DocenteBean)
   * @param d  (Date)
   * @param ti (Time)
   * @param tf (Time)
   * @return arrsb(nel try, se riesce a stampare tutti gli studenti) oppure
   *         null(nel catch, altrimenti)
   */
  
  public synchronized ArrayList<StudenteBean> getStudentiAssenza(
      DocenteBean db, Date d, Time ti, Time tf) {
    ...
    try {
      ...
      conn = DriverManagerConnectionPool.getConnection();
      ps = conn.prepareStatement(
          "SELECT s.nome, s.cognome, s.email, s.matricolaStud, s.password "
          + "FROM prenota p, studente s "
          + "WHERE s.email = p.prenotaEmailStud AND p.prenotaMatricolaDoc = ? AND p.giorno = ? "
   ...
   * @return true (nel try, nel caso in cui l'ha rimosso con successo) oppure
   *         false(nel catch, nel caso in cui la cancellazione non è avvenuta con
   *         successo).
   */
   ...
   * @param email (String)
   * @return db (DocenteBean) nel try, nel caso in cui l'ha modificato con successo
   *         e ritorna null se non corrisponde il controllo sulla password,oppure
   *         null nel catch, nel caso in cui la modifica non è avvenuta con
   *         successo).
   */
  public DocenteBean modificaAccount(String nome, String cognome, String matricola,
      String studio, String oldPassword, String newPassword, String immagine, String email) {
    Connection conn = null;
    PreparedStatement ps = null;
    ...
    try {
      ...
      if (oldPassword.equals(dbPass)) {
        ps1 = conn.prepareStatement(
            "UPDATE docente SET nome = ?, cognome = ?, password = ?, studio = ?, immagine = ?"
            + " WHERE matricolaDoc = ?;");
