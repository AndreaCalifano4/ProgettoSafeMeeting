...
public class CorsoDAO {
	 ...
	 * @param cb (CorsoBean)
	 * @return arr (nel try, cioè l'arraylist di tutti i corsi) oppure null (nel catch, altrimenti)
	 */
	 ...
	 * @return cb(CorsoBean, se l'operazione è andata a buon fine) oppure null (in tutti gli altri casi)
	 */
	...
	public synchronized CorsoBean associaCorso(InsegnaBean ib)
	{
		...
		{
			con = DriverManagerConnectionPool.getConnection();
			ps = con.prepareStatement("SELECT * FROM insegna WHERE insegnaMatricolaDoc = ? AND insegnaCodiceCorso = ?");
	 ...
	 * @return arr(nel try, ArrayList di tutti i corsi) oppure null (nel catch, altrimenti)
	 */
	public synchronized ArrayList<CorsoBean> visualizzaCorsi(DocenteBean db) {
			...
			{
				con = DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("SELECT c.nome, c.num_CFU, c.ore_totali,c.codice FROM docente d, corso c, insegna i WHERE d.matricolaDoc = i.insegnaMatricolaDoc AND i.insegnaCodiceCorso= c.Codice AND d.cognome=?");
	...
	public synchronized boolean eliminaCorso(InsegnaBean ib) {
	    ...
	    {
	      con = DriverManagerConnectionPool.getConnection();
	      ps = con.prepareStatement("DELETE FROM insegna WHERE insegnaMatricolaDoc=? AND insegnaCodiceCorso=?");
	      ps.setString(1, ib.getInsegnaMatricolaDoc());
	      ps.setString(2, ib.getInsegnaCodiceCorso());
