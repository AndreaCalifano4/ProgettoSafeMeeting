...
public class StudenteDAO {
	 ...
	 * @return sb (StudenteBean, nel try, in caso il login sia riuscito) oppure null(nel try, altrimenti) oppure e.printStackTrace()(nel catch, in caso di errore)
	 */
	 ...
	 * @return null(nel try, in caso non ci siano docenti) oppure listadoc(nel try, contenente i docenti ricarcati) oppure e.printStackTrace()(nel catch, in caso di errore)
	 */
	public ArrayList<DocenteBean> ricercaDocente(String param,DocenteBean db){
		...
		try
		{
			...
			conn = DriverManagerConnectionPool.getConnection();
			ps = conn.prepareStatement("SELECT * FROM docente d, corso c, insegna i WHERE d.matricolaDoc=i.insegnaMatricolaDoc AND i.insegnaCodiceCorso = c.Codice AND d.cognome LIKE ? LIMIT 1");
			...
			if(listadoc.isEmpty()) {
				ps2 = conn.prepareStatement("SELECT * FROM docente d, corso c, insegna i WHERE d.matricolaDoc=i.insegnaMatricolaDoc AND i.insegnaCodiceCorso = c.Codice AND c.nome LIKE ?");
	 ...
	 * @return db(DocenteBean, nel try, in caso l'operazione sia riuscita) oppure null(nel catch, altrimenti)
	 */
	 ...
	 * @param matricola (matricola del docente)
	 * @return arrcb(nel try, l'arraylist di tutti i corsi del docente) oppure null(nel catch, altrimenti)
	 */
	public ArrayList<CorsoBean> getCorso(String matricola) {
		...
		try 
		{
			...
			conn = DriverManagerConnectionPool.getConnection();
			ps = conn.prepareStatement("SELECT c.nome FROM docente d, corso c, insegna i WHERE d.matricolaDoc = i.insegnaMatricolaDoc AND i.insegnaCodiceCorso = c.Codice AND d.matricolaDoc = ?");
			ps.setString(1, matricola);
	 ...
	 * @param matricola (matricola del docente)
	 * @return arrrb(nel try, l'arraylist di tutti i ricevimenti del docente) oppure false(nel catch, altrimenti)
	 */
	public ArrayList<RicevimentoBean> getRicevimento(String matricola) {
		...
		try 
		{
			...
			conn = DriverManagerConnectionPool.getConnection();
			ps = conn.prepareStatement("SELECT r.giorno, r.ora_inizio, r.ora_fine FROM docente d, ricevimento r WHERE d.matricolaDoc = r.ricevMatricolaDoc AND d.matricolaDoc = ?");
			ps.setString(1, matricola);
	 ...
	 * @param matricola (matricola del docente)
	 * @return arrtb(nel try, l'arraylist di tutti le tipologie) oppure null(nel catch, altrimenti)
	 */
	public ArrayList<TipologiaBean> getTipologia(String matricola) {
		...
		try 
		{
			...
			conn = DriverManagerConnectionPool.getConnection();
			ps = conn.prepareStatement("SELECT * FROM tipologia t, docente d WHERE t.tipoMatricolaDoc = d.matricolaDoc AND d.matricolaDoc = ?");
			ps.setString(1, matricola);
	...
	/**
	 * Questo metodo serve per ricavarci dal db i dati del messaggio, dato un determinato studente
	 * @param email(String)
	 * @return arrmb(ArrayList<MessaggioBean>) nel caso in cui la query è stata effettuata con successo, oppure ritorna null nel catch in caso di eccezioni.
	 */
	...
	/**
	 * Questo metodo serve per ricavare i dati del docente che hanno segnalato un'assenza e, quindi, creato un messaggio;
	 * @param email(String)
	 * @return arrmb(ArrayList<DocenteBean>) nel caso in cui la query è stata effettuata con successo, oppure ritorna null in caso di eccezioni.
	 */
	public synchronized ArrayList<DocenteBean> getDocMessaggio(String email) {
		...
		try {
			conn = DriverManagerConnectionPool.getConnection();
			ps = conn.prepareStatement("SELECT d.nome,d.cognome FROM messaggio m, docente d WHERE m.messaggioMatricolaDoc = d.matricolaDoc AND m.messaggioEmailStud = ? ");
			ps.setString(1, email);
	...
	/**
	 * Questo metodo ci permette di cancellare un'account di uno studente all'interno del db;
	 * @param email(String)
	 * @return true (nel try, nel caso in cui l'account dello studente viene eliminato con successo) oppure false (nel catch, altrimenti)
	 */
	...
	/**
	 * Questo metodo ci permette di modificare i dati dello studente presenti nel db;
	 * @param nome(String)
	 ...
	 * @param email(String)
	 * @return sb(StudenteBean) nel try, nel caso in cui l'ha modificato con successo e ritorna null se non corrisponde il controllo sulla password,oppure null nel catch, nel caso in cui la modifica non è avvenuta con successo).
	 */
	public StudenteBean modificaAccount(String nome, String cognome, String matricola, String oldPassword, String newPassword,String email)
	{
		Connection conn = null;
		PreparedStatement ps = null;
		...
		try
		{
			...
			{
				ps1 = conn.prepareStatement("UPDATE studente SET matricolaStud = ?, nome = ?, cognome = ?, password = ? WHERE email = ?;");
