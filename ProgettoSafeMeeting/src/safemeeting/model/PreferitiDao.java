...
public class PreferitiDao {
   ...
   * @return null (se la lista è vuota) oppure listadoc (se la lista contiene
   *         elementi, nel try) oppure SQLException (nel catch, in caso di errore)
   */
  public ArrayList<DocenteBean> getPreferiti(String preferitiEmailStud) {
    ...
    try {
      ...
      ps = conn.prepareStatement("SELECT doc.matricolaDoc, doc.nome, doc.cognome, doc.immagine "
          + "FROM docente AS doc, aggiungi_ai_preferiti AS preferiti " + "WHERE "
   ...
   * @param preferitiEmailStud (email dello studente)
   * @return true (nel try, se l'operazione Ã¨ andata a buon fine) oppure false
   *         (nel try, altrimenti) oppure SQLException (nel catch, in caso di
   *         errore)
   */
  public synchronized boolean setPreferiti(String matricolaDoc, String preferitiEmailStud) {
    ...
    try {
      ...
      ps2 = conn.prepareStatement(
          "select * from aggiungi_ai_preferiti "
          + "where preferitiEmailStud = ? AND preferitiMatricolaDoc = ?");
   ...
   * @return true (nel try, in caso l'operazione vada a buon fine) oppure
   *         false(nel catch, altrimenti)
   */
  public synchronized boolean deletePreferiti(String matricolaDoc, String preferitiEmailStud,
      ArrayList<DocenteBean> db) {
    Connection conn = null;
    PreparedStatement ps = null;
    ...
    try {
      ...
      ps = conn.prepareStatement(
          "DELETE FROM aggiungi_ai_preferiti "
          + "WHERE preferitiMatricolaDoc = ? AND preferitiEmailStud = ?");
      ps.setString(1, matricolaDoc);
      ps.setString(2, preferitiEmailStud);
