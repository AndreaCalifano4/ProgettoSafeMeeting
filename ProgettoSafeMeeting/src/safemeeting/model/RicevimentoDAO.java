...
public class RicevimentoDAO {
	 ...
	 * @return true (nel try, se è stato creato con successo) oppure false (nel try, se non è stato creato) oppure false (nel catch, per colpa di qualche errore) 
	 */
	public synchronized boolean creaRicevimento(RicevimentoBean rb)
	{
		...
		try
		{
			...
			ps3 = conn.prepareStatement("select count(ricevMatricolaDoc) AS conta from ricevimento where ricevMatricolaDoc = ?");
			...
			if(cont<2) {
				ps = conn.prepareStatement("select * from ricevimento where ricevMatricolaDoc = ? AND ora_inizio= ? AND ora_fine= ? AND giorno= ?");
	 ...
	 * @return true (nel try, se la tipologia è stata creata) oppure false (nel catch, altrimenti)
	 */
	public synchronized boolean creaTipologia(TipologiaBean tb) {
		...
		try {
			...
			if(!res.next()) {
				...
				ps3 = con.prepareStatement("UPDATE tipologia SET tempo = ? WHERE tipoMatricolaDoc = ? AND tipo = ?");
	 ...
	 * @return arrb (nel try, stampa l'arraylist contenente tutti i ricevimenti) oppure e.printStackTrace()(nel catch, in caso ci sia un errore)
	 */
	...
	/**
	 * Questo metodo serve per eliminare un determinato ricevimento all'interno del db;
	 * @param rb(RicevimentoBean)
	 * @return true (nel try, nel caso in cui il ricevimento viene eliminato con successo) oppure false (nel catch, altrimenti)
	 */
	...
	public synchronized boolean eliminaRicevimento(RicevimentoBean rb){
		...
		try {
			...
			con = DriverManagerConnectionPool.getConnection();
			ps = con.prepareStatement("delete from ricevimento where ricevMatricolaDoc = ? and ora_inizio = ? and ora_fine = ? and giorno = ?");
			ps.setString(1, rb.getMatricolaDoc());
			ps.setTime(2, rb.getOra_inizio());
			ps.setTime(3, rb.getOra_fine());
			ps.setString(4, rb.getGiorno());
