...
public class PrenotaDAO {
	 ...
	 * @return true (nel try, in caso l'operazione abbia successo) oppure false (nel catch, altrimenti)
	 */
	...
	public synchronized boolean effettuaPrenotazione(PrenotaBean pb) {
		...
		public synchronized boolean settaTempo(PrenotaBean pb,ArrayList<RicevimentoBean> rb) {
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("SELECT p.orario,p.tipologia FROM prenota p WHERE p.numero_prenotazione = ( SELECT max(p.numero_prenotazione) FROM prenota p WHERE p.prenotaMatricolaDoc = ? AND p.giorno = ?);");
				...
				if(!res.next()) {
					...
					ps2 = con.prepareStatement("SELECT t.tempo FROM tipologia t, docente d, prenota p WHERE t.tipoMatricolaDoc = d.matricolaDoc AND d.matricolaDoc = p.prenotaMatricolaDoc AND t.tipo = ? AND d.matricolaDoc = ? ");
					...
					if(res2.next()) {
						...
						if(rb.get(0).getOra_fine().toLocalTime().getHour() <= tempoeffett.toLocalTime().getHour()) {
							if(rb.get(1).getOra_fine().toLocalTime().getHour() <= tempoeffett.toLocalTime().getHour()) {
								return false;
							}
							else {
								if(rb.get(0).getOra_fine().toLocalTime().getHour() == tempoeffett.toLocalTime().getHour()) {
		...
		/**
		 * Questo metodo ritorna il codice identificativo della prenotazione per poterla stampare a video;
		 * @param sb(StudenteBean)
		 * @return arrpb (ArrayList<PrenotaBean>) nel try in caso in cui la query è stata effettuata con successo, oppure ritorna null in caso di eccezioni nel catch.
		 */
		...
		public synchronized ArrayList<PrenotaBean> getNumPrenotazioni(StudenteBean sb){
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("select p.numero_prenotazione, p.prenotaMatricolaDoc from prenota p where p.prenotaEmailStud= ?");
		...
		/**
		 * Questo metodo serve per ritornare i dati del docente data una determinata prenotazione, per poterli stampare a video;
		 * @param sb(StudenteBean)
		 * @param numprenot(int)
		 * @return db(DocenteBean) nel caso in cui la query è stata eseguita con successo, oppure ritorna null nel catch in caso di eccezioni.
		 */
		public synchronized DocenteBean getDocentePrenot(StudenteBean sb, int numprenot){
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("select d.nome, d.cognome from docente d, prenota p where p.prenotaMatricolaDoc=d.matricolaDoc AND p.prenotaEmailStud = ? AND p.numero_prenotazione = ?");
				ps.setString(1, sb.getEmail());
		...
		/**
		 * Questo metodo serve per recupare dal db i dati di una determinata prenotazione per poterseli stampare a video;
		 * @param sb(StudenteBean)
		 * @param numprenot(int)
		 * @return pb(PrenotaBean) in caso in cui la query è stata effettuata con successo, oppure ritorna null nel catch in caso di eccezioni.
		 */
		public synchronized PrenotaBean getDatiPrenot(StudenteBean sb, int numprenot){
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("select p.numero_prenotazione, p.tipologia, p.orario, p.giorno from docente d, prenota p where p.prenotaMatricolaDoc=d.matricolaDoc AND p.prenotaEmailStud = ? AND p.numero_prenotazione = ?");
				ps.setString(1, sb.getEmail());
				ps.setInt(2, numprenot);
		 ...
		 * @return i(int) nel caso in cui la query è stata effettuata con successo, in caso contrario ritorna 0, oppure ritorna 0 nel catch nel caso in cui sono presenti eccezioni.
		 */
		public synchronized int getNumPrenotati(String matricoladocente) {
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("select count(numero_prenotazione) from prenota p,docente d where p.prenotaMatricolaDoc=d.matricolaDoc AND p.prenotaMatricolaDoc = ?"); 
		 ...
		 * @param sb(StudenteBean)
		 * @param numprenot (int)
		 * @return true (nel try, nel caso in cui il corso viene eliminato con successo) oppure false (nel catch, altrimenti)
		 */
		...
		public synchronized boolean eliminaPrenotazione(StudenteBean sb, int numprenot) {
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("delete from prenota where numero_prenotazione = ? AND prenotaEmailStud = ?"); 
		...
		/**
		 * Questo metodo ritornerà la lista degli studenti prenotati per un singolo docente;
		 * @param pb(PrenotaBean)
		 * @param db(DocenteBean)
		 * @return arrpb(ArrayList<PrenotaBean>) nel caso in cui la query è stata effettuata con successo ed ha ritornato dei valori, oppure ritorna null in caso di eccezioni.
		 */
		public synchronized ArrayList<PrenotaBean> listaPrenotatiDoc(PrenotaBean pb , DocenteBean db) {
			...
			try {
				con =  DriverManagerConnectionPool.getConnection();
				ps = con.prepareStatement("select s.nome, s.cognome, s.matricolaStud, p.nome_corso, p.giorno, s.email, p.tipologia,p.orario from studente s, prenota p where s.email = p.prenotaEmailStud AND p.prenotaMatricolaDoc = ?");
